...
public class DataFrameworkCore {
	...
	/**
	 * Abre una conexion a ADF generando todos los objetos a partir de los
	 * archivos XML pasados como parametros
	 * 
	 * @param context
	 *            Actividad principal
	 * @param idTables
	 *            recurso XML con las tablas
	 * @param idInitialValues
	 *            recurso XML con los registros a generar en la primera carga
	 * 
	 */
	public void open(Context context, String namePackage,
			ArrayList<Table> tables) throws XmlPullParserException, IOException {
		try {
			// Solo en el caso de que no estï¿½n cargadas las tablas, leemos el
			// XML
			if (tables.size() == 0) {
	...
	/**
	 * Genera los valores iniciales de la primera carga en el caso que se haya
					...
					if (x.getName().equals("row")) {
						...
						if (x.getAttributeValue(null, "table") != null) {
							...
							if (x.getAttributeValue(null, "id") != null) {
								id = Integer.parseInt(x.getAttributeValue(null,
										"id"));
							}
	...
	private static final String TABLE_RAW_PREFIX = "TABLE_DATA_";

	@SuppressLint("DefaultLocale")
	private void initRaws(SQLiteDatabase db) {
		try {
			String rClassName = getPackage() + ".R";
			String rawClassName = rClassName + ".raw";
			java.lang.reflect.Field[] fields = null;
			Class<?>[] clzzz = Class.forName(rClassName).getClasses();
			for (Class<?> class1 : clzzz) {
				if (rawClassName.equals(class1.getCanonicalName())) {
			...
			if (fields != null) {
				for (java.lang.reflect.Field field : fields) {
					if (field.getName().toUpperCase()
							.startsWith(TABLE_RAW_PREFIX)) {
						importRaw(db, field.getName());
					}
				}
			}
		} catch (ClassNotFoundException e) {
			Log.e("Importing Data Error", e.getMessage(), e);
		}
	}

	private void importRaw(SQLiteDatabase db, String rawName) {
		BufferedReader in = null;
		try {
			String tableName = rawName.substring(TABLE_RAW_PREFIX.length());
			int rawID = mCtx.getResources().getIdentifier(
					mPackage + ":raw/" + rawName, null, null);
			InputStream inputStream = mCtx.getResources()
			...
			while ((reader = in.readLine()) != null) {
				...
				for (String column : columnNames) {
					values.put(column, rowData[i++]);
				}
				db.insert(tableName, null, values);

			}
		} catch (IOException e) {
			Log.e("Error while importing data, check if separetor on the file "
					+ rawName + " are pipe«s \"|\" ", e.getMessage(), e);
		} finally {
			if (in != null) {
				try {
					in.close();
